...
const socketio = (() => {
    ...
    const boxPayloadMap = map(([id, payload]) => payload.boxs.map(({ id, box, style = {} }) => {
        Object.entries(box).forEach(([key, value]) => (style[key] = 100 * value + "%"));
        return {
            id,
            style,
    ...
    const boxPayloadObservable = (_id) => boxEvent.pipe(filter(([id, payload]) => _id === id), boxPayloadMap);
...
export class CCTVBBox extends HTMLElement {
    ...
    connectedCallback() {
        this.defaultBorderColor = getComputedStyle(document.body).getPropertyValue("--default-box-border");
        ...
        const observableDisconnect = from(new Promise((resolve) => {
